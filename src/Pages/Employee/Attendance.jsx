  ...
  export default function AttendancePage() {
    ...
    const handleAddEntry = () => {
      ...
      if (existing) {
        setLogs(
          logs.map((log) =>
            log.date === date
              ? { ...log, inTime, outTime: outTime || null, note }
              : log
          )
        );
      } else {
        setLogs(
          [...logs, { date, inTime, outTime: outTime || null, note }].sort(
    ...
    const saveEditing = (rowDate) => {
      setLogs(
        logs.map((log) =>
          log.date === rowDate ? { ...log, note: editText } : log
        )
      );
      setEditingDate(null);
      setEditText("");
          ...
          <table className="w-full text-sm">
            ...
            <tbody>
              ...
              {logs.map((log) => {
                ...
                return (
                  <tr key={log.date} className="border-b border-slate-100 last:border-0 align-top">
